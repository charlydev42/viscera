// ============================================================
// FLUBBER BLOB V7f — LIGHTER BASE + SLIGHTLY LESS BOOST
// ============================================================
// Same as V7e but:
// - Rest color is brighter/lighter green
// - Audio reaction ~20% less intense on blob movement
// ============================================================
...
float map(vec3 p) {
    ...
    // Audio — slightly reduced multipliers (was *3.0, now *2.5 etc)
    ...
    // CORE
    vec3 coreRadii = vec3(
        1.3 + sb * 0.25 + bss * 0.12,
        0.65 - bss * 0.06 + md * 0.16,
        1.0 + sb * 0.16 + hgs * 0.04
    ) * S;
    float core = sdEllipsoid(p, coreRadii);

    // Blob 1 — bass (orbit reduced ~20%)
    float b1Orbit = (0.3 + bss * 1.0) * S;
    vec3 b1Pos = vec3(
    ...
    d = smin(d, blob2, k);
    d = smin(d, blob3, kHigh);
    d = smin(d, blob4, k);

    if (loudness > 0.35) {
        d = smin(d, blob5, k * 0.7);
        d = smin(d, blob6, k * 0.7);
    }
    if (extremeLoud > 0.15) {
        d = smin(d, blob7, kHigh * 0.8);
        d = smin(d, blob8, k * 0.65);
    }
...
float D_GGX(vec3 N, vec3 H, float r) {
    float a2=r*r*r*r; float d=max(dot(N,H),0.0); d=d*d*(a2-1.0)+1.0;
    return a2/(PI*d*d);
}
...
vec3 fluidColor(vec3 p, vec3 N, vec3 V, float t) {
    ...
    // =============================================
    // LIGHTER rest palette — bumped up ~30-40%
    // =============================================
    vec3 coolA = vec3(0.20, 0.48, 0.15);   // Was 0.10, 0.30, 0.08
    vec3 coolB = vec3(0.42, 0.72, 0.25);   // Was 0.30, 0.60, 0.15
    vec3 coolC = vec3(0.28, 0.58, 0.42);   // Was 0.15, 0.45, 0.35

    // Active: brand lime (unchanged)
